<script lang="ts">
    ...
    let modeCurrent = localStorageStore<string>("theme", "light");
    // Classes
    ...
    function onToggleHandler(): void {
        $modeCurrent = $modeCurrent === "dark" ? "light" : "dark";
    ...
    // State
    $: trackBg = $modeCurrent === "light" ? bgLight : bgDark;
    $: thumbBg = $modeCurrent === "light" ? bgDark : bgLight;
    $: thumbPosition = $modeCurrent === "light" ? "translate-x-[100%]" : "";
    $: iconFill = $modeCurrent === "light" ? fillLight : fillDark;
    // Reactive
...
>
    ...
    <div class="lightswitch-thumb {classesThumb}">
        ...
        <svg
            ...
            viewBox="0 0 512 512"
        >
            <path d={$modeCurrent === "light" ? svgPath.sun : svgPath.moon} />
        </svg>
    </div>
</div>
